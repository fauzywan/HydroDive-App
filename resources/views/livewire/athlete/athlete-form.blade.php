<div>
    <x-auth-header class="mb-2" :title="__($modalText)" :description="__($modalSubText)" />

    <div class="flex justify-center item-center mt-2">
            <form class="overflow-hidden"  wire:submit="{{$this->formType}}">
                <div class=" mb-5 w-[300%] flex md:gap-[5em] sm:gap-[2em] "style=" transform:translateX(-{{$formStage*34}}%)">
                ...
                <div class="second-stage w-full ml-2">
                    ...
                    @endif
                    @if (auth()->user()->role_id==5 && $formType=="save")

                    <flux:field>
                        <flux:label>Registration Fee</flux:label>
                        <flux:description>Uang Pendaftaran Peserta Didik</flux:description>
                        <flux:input laceholder="" wire:model="registration_fee"  />
                ...
                <div class="flex mt-2 {{$formStage==2?'justify-center items-center':'justify-end'}} gap-5 mr-2">
                    @if($formStage>0)
